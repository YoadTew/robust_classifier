...
from data.color.data_manager import get_val_loader, get_train_loader
...
class Trainer:
    ...
    def _do_epoch(self, epoch_idx):
        ...
        for batch_idx, (images, targets, colorized_imgs) in enumerate(self.train_loader):
            images, targets, colorized_imgs = images.to(self.device), targets.to(self.device), colorized_imgs.to(self.device)

            self.optimizer.zero_grad()
            outputs, imgs_logits = self.model(images)
            colorized_imgs_outputs, colorized_imgs_logits = self.model(colorized_imgs)
            # outputs = torch.squeeze(outputs)

            cls_loss = self.criterion(outputs, targets)
            color_loss = self.color_criterion(imgs_logits, colorized_imgs_logits)

            loss = cls_loss + self.args.color_loss_weight * color_loss

            if batch_idx % 30 == 1:
                print(f'epoch:  {epoch_idx}/{self.args.epochs}, batch: {batch_idx}/{len(self.train_loader)}, '
                      f'loss: {loss.item()}, cls_loss: {cls_loss.item()}, color_loss: {color_loss.item()}')
            ...
            self.writer.add_scalar('color_loss_train', color_loss.item(), epoch_idx * len(self.train_loader) + batch_idx)
