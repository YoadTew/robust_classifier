...
class ShapeNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.projection = nn.Conv2d(3, 3, kernel_size=7, padding=3, bias=True)
    ...
    def _forward_impl(self, x, use_projection):
        ...
        activations['maxpool_1'] = x

        x = self.layer1(x)
        activations['layer_1'] = x
        x = self.layer2(x)
        activations['layer_2'] = x
        x = self.layer3(x)
        activations['layer_3'] = x
        x = self.layer4(x)
        activations['layer_4'] = x

        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        activations['representation'] = x
...
def shapenet18(pretrained=False, progress=True, num_classes=1000, **kwargs):
